...
public class SpeechToTextPlugin :
    ...
    private fun startListening(result: Result, languageTag: String, partialResults: Boolean,
        ...
        handler.post {
            run {
                // PATCHED (vendored fork — see /third_party/speech_to_text
                // at the repo root for why): speechRecognizer.startListening()
                // binds to whatever the OS resolves as the device's speech
                // recognition service, which on some OEM devices is a
                // restricted/non-exported component that throws
                // SecurityException("Not allowed to bind to service...")
                // right here. This runs on a posted Handler callback, well
                // outside the onMethodCall try/catch above and outside
                // anything Dart-side can ever catch — unhandled, it takes
                // the whole app down. Route it through the plugin's own
                // sendError() (-> onError on the Dart side) instead, same
                // as every other recognizer failure this plugin reports.
                try {
                    speechRecognizer?.startListening(recognizerIntent)
                } catch (exc: Exception) {
                    Log.e(logTag, "startListening failed", exc)
                    sendError("${SpeechToTextErrors.unknown.name}: ${exc.javaClass.simpleName}: ${exc.message}")
